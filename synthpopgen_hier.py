...
def run_hierarchical_by_base(df_all: pd.DataFrame, base_col: str, display_mode: str = "split") -> pd.DataFrame:
    """
    ...
    for base_val, g in df_all.groupby(base_col, dropna=False):
        ...
        # If there are no inner constraints, skip (or return empty)
        if len(df_inner) == 0:
            continue
        ...
        # Ensure column order matches original convention
...
Examples:
  ...
  python synthpopgen.py -i input_file_tuples.csv -f "gender:female" -o female_results.csv
    ...
    try:
        ...
        if 'ASL' in df_tuples.columns and args.filter.lower() == 'all':
            synthetic_df = run_hierarchical_by_base(df_tuples, base_col='ASL', display_mode=args.display)
        else:
