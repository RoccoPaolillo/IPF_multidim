...
# Print total
print(f"Total estimated_count for rows with {target_components}: {total_estimated_count}")

# Optional: Save matching rows with total appended
matching_rows_with_total = pd.concat([
    matching_rows,
    pd.DataFrame([{'combination': 'TOTAL', 'estimated_count': total_estimated_count}])
])

matching_rows_with_total.to_csv("filtered_combinations.csv", index=False)