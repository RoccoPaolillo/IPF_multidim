...
def syntheticextraction(df_tuples, target_components, display_mode='split', total_population=None):
    """
    ...
    if total_population is None:
        ...
        if total_population is None:
            raise ValueError(
                "Cannot infer total_population: no 1D marginal rows found "
                "(rows with exactly one filled dimension). "
                "If you are running hierarchical synthesis, pass total_population explicitly."
            )
    else:
...
def compute_ape_by_empirical_constraints(df_tuples, synthetic_df, eps=1e-12):
    """
...
def _normalize_missingness(df: pd.DataFrame) -> pd.DataFrame:
    """Strip whitespace and convert empty strings to NaN for all object columns."""
    df = df.copy()
    for c in df.columns:
...
def run_hierarchical_by_base(df_all: pd.DataFrame, base_col: str, target_components_inner, display_mode: str = "split", base_filter=None) -> pd.DataFrame:
    """
    Run synthesis separately within each base_col group (e.g., ASL), using the base-only row as total_population.

    Parameters
    ----------
    df_all : DataFrame
        Full constraint table including base_col and inner dimensions plus 'value'.
    base_col : str
        Name of the hierarchical base column (e.g., 'ASL').
    target_components_inner : list
        Target tuple for INNER dimensions only (order must match inner_dims). Use ["all"] for full-joint.
    display_mode : {"split","aggregate"}
        Passed through to syntheticextraction.
    base_filter : str or None
        If provided, run only that base group (e.g., a single ASL name).

    Expected structure per base group:
      - exactly one base-only row: base_col filled, all other dimensions empty/NaN (value = total of that base)
      - all other constraint rows include base_col plus 1+ inner dimensions
    """
    if base_col not in df_all.columns:
        raise ValueError(f"Hierarchical mode requested but '{base_col}' column not found.")

    dim_cols = [c for c in df_all.columns if c != "value"]
    inner_dims = [c for c in dim_cols if c != base_col]

    out_parts = []

    grouped = df_all.groupby(base_col, dropna=False)
    for base_val, g in grouped:
        if pd.isna(base_val) or str(base_val).strip() == "":
            continue
        if base_filter is not None and str(base_val).strip() != str(base_filter).strip():
            continue

        g = g.copy()

        # base-only total row: base_col present, all inner dims missing
        mask_total = g[base_col].notna()
        for c in inner_dims:
            mask_total &= g[c].isna()

        n_tot = int(mask_total.sum())
        if n_tot != 1:
            raise ValueError(
                f"{base_col}='{base_val}': expected exactly 1 '{base_col}-only' total row (value=total population), found {n_tot}."
            )

        N_base = int(g.loc[mask_total, "value"].iloc[0])

        # Inner constraints: drop base column; base-conditioned marginals become 1D marginals here
        df_inner = g.loc[~mask_total, inner_dims + ["value"]].copy()

        if len(df_inner) == 0:
            continue

        syn_inner = syntheticextraction(
            df_inner,
            target_components=target_components_inner,
            display_mode=display_mode,
            total_population=N_base
        )
        syn_inner[base_col] = str(base_val).strip()

        # Column order: base, then inner dims, then value
        syn_inner = syn_inner[[base_col] + inner_dims + ["value"]]

        out_parts.append(syn_inner)

    if not out_parts:
        return pd.DataFrame(columns=[base_col] + inner_dims + ["value"])
...
Examples:
  ...
  python synthpopgen.py -i input_file_tuples.csv -f "gender:female" -o female_results.csv
    ...
    try:
        ...
        df_tuples = _normalize_missingness(df_tuples)
        ...
        if 'ASL' in df_tuples.columns:
            # Hierarchical mode: run synthesis within each ASL.
            dim_cols = [c for c in df_tuples.columns if c != 'value']
            asl_idx = dim_cols.index('ASL')
            asl_target = None
            if isinstance(target_components, (list, tuple)) and target_components != ['all']:
                asl_target = target_components[asl_idx]

            # Inner target tuple excludes ASL
            if target_components == ['all']:
                inner_target = ['all']
            else:
                inner_target = [t for i, t in enumerate(target_components) if i != asl_idx]

            synthetic_df = run_hierarchical_by_base(
                df_tuples,
                base_col='ASL',
                target_components_inner=inner_target,
                display_mode=args.display,
                base_filter=asl_target
            )

            # If ASL is not constrained and user asked for aggregate, also aggregate across ASL
            if asl_target is None and args.display == 'aggregate':
                inner_dims = [c for c in dim_cols if c != 'ASL']
                if len(synthetic_df) > 0:
                    synthetic_df = (
                        synthetic_df
                        .assign(ASL='')
                        .groupby(['ASL'] + inner_dims, as_index=False)['value']
                        .sum()
                    )
        else:
        ...
        if args.validate:
            ...
            ape_df = compute_ape_by_empirical_constraints(df_tuples, synthetic_df)

            base = args.validate.rsplit(".", 1)[0]
            rmse_path = base + "_RMSE.csv"
            ape_path = base + "_APE.csv"
            for _p in (rmse_path, ape_path):
                try:
