...
class SynthPopGUI(tk.Tk):
    ...
    def __init__(self):
        ...
        logo_path = "extra/FOSSR handbook-SPG_RP.jpeg"
        try:
        ...
        # --- Display mode ---
        tk.Label(self, text="3) Subcategories split or aggregated:").grid(
        ...
        # --- Validation options ---
        tk.Label(self, text="4 Optional) Validation (only with filter=all):").grid(
        ...
        # --- synth-total option ---
        tk.Label(self, text="5 Optional) Rescale synthetic population (only with filter=all):").grid(
            row=5, column=0, sticky="w", padx=5, pady=5
        )
        self.synth_total_entry = tk.Entry(self, width=20)
        self.synth_total_entry.grid(row=5, column=1, sticky="w")
        tk.Label(self, text="(leave blank to use input total)").grid(
    ...
    def generate_and_save_barplot_percent(self):
        ...
        png_path = os.path.join(out_dir, f"{base}_barplot.png")
        ...
        ax.barh(labels,percentages.tolist())
        ax.set_xlabel("% share")
